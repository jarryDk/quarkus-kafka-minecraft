...
----
The gradle build use `shadowJar` to inlude all dependencies into our `kafka-1.0.7.jar` file.
...
script build the binary and deploy to the folder `$FORGE_SERVER_LOCATION/mods/kafka-1.0.7.jar` - no housekeep when
