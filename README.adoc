...
----
The gradle build use `shadowJar` to inlude all dependencies into our `kafka-1.0.4.jar` file.
Included is `org.apache.kafka:kafka-clients:3.6.1` and `com.fasterxml.jackson.core:jackson-databind:2.16.0`.
----
...
An alternative to use `shadowJar` is to add the dependencies (five jar-files) manually one my one in the section `-DlegacyClassPath=` in the file
    ...
    /opt/minecraft/forge/.minecraft_1.20.4-49.0.11/libraries/net/minecraftforge/forge/1.20.4-49.0.11/unix_args.txt


I use link:minecraft-kafka-mod/build_and_deploy.sh[minecraft-kafka-mod/build_and_deploy.sh] to deploy `minecraftforge`. The
script build the binary and deploy to the folder `$FORGE_SERVER_LOCATION/mods/kafka-1.0.4.jar` - no housekeep when
...
[source,bash]
----
export KAFKA_HOME=/opt/apache/kafka/kafka_2.13-3.6.1
----
